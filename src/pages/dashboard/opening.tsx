...
const Opening: FC<openingProps> = ({ days }: openingProps) => {
  ...
  const { mutate: saveOpeningHours, isLoading } = api.opening.changeOpeningHours.useMutation({
              ...
              <div className='grid grid-cols-3 place-items-center' key={day.id}>
                ...
                <div className='mx-4'>
                  <TimeSelector
                    ...
                    selected={
                      openingHours[
                        openingHours.findIndex((x) => x.name === weekdayIndexToName(day.dayOfWeek))
                      ]?.openTime
                    }
                  />
                </div>
...
function getOpeningHours(days: Day[]) {
  const openingHours: OpeningHoursType[] = weekdays.map((day: string, i): OpeningHoursType => {
    return {
      name: day,
